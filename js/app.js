...
async function shareItemCard(it) {
  try {
    ...
    g.addColorStop(0.55, "#b91c2e");
    g.addColorStop(1, "#6f0e1d");
    ctx.fillStyle = g;
    ctx.fillRect(0, 0, W, H);
    ctx.fillStyle = "rgba(255,255,255,.06)";
    ctx.beginPath(); ctx.arc(W - 80, 80, 200, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.arc(60, H - 110, 170, 0, Math.PI * 2); ctx.fill();
    // 顶部品牌行
    ctx.textAlign = "center";
    ctx.fillStyle = "#fde8b0";
    ctx.font = "bold 24px sans-serif";
    ctx.fillText("洪合羊毛衫 · 厂家直供", W / 2, 60);
    // 款式名
    ctx.fillStyle = "#ffffff";
    ctx.font = "bold 46px sans-serif";
    ctx.fillText(it.name, W / 2, 116);
    // 图片（白底圆角）
    const img = new Image();
    ...
    ctx.fillStyle = "#ffffff";
    roundRectPath(ctx, 40, 148, W - 80, 500, 26);
    ctx.fill();
    if (img.width) {
      const iw = W - 80, ih = 500;
      const ratio = Math.min(iw / img.width, ih / img.height);
      const dw = img.width * ratio, dh = img.height * ratio;
      ctx.save();
      roundRectPath(ctx, 40, 148, W - 80, 500, 26);
      ctx.clip();
      ctx.drawImage(img, (W - dw) / 2, 148 + (500 - dh) / 2, dw, dh);
      ctx.restore();
    } else {
    ...
    ctx.fillText("¥" + fmt(it.price), W / 2, 726);
    // 库存/已售（不显示成本）
    ctx.font = "26px sans-serif";
    ctx.fillStyle = "rgba(255,255,255,.85)";
    ctx.fillText("库存 " + totalStock + " 件" + (totalSold ? " · 已售 " + totalSold + " 件" : "") + " · 现货速发", W / 2, 782);
    // 颜色标签（圆角小片）
    const colors = (it.colors || []).slice(0, 6);
    if (colors.length) {
      ctx.font = "24px sans-serif";
      const widths = colors.map((c) => ctx.measureText(c.name + " " + c.stock).width);
      const totalW = widths.reduce((a, b) => a + b, 0) + colors.length * 48 - 16;
      let cx = (W - totalW) / 2;
      const cy = 828;
      for (let i = 0; i < colors.length; i++) {
        ctx.fillStyle = "rgba(255,255,255,.16)";
        roundRectPath(ctx, cx, cy, widths[i] + 32, 46, 23);
        ctx.fill();
        ctx.fillStyle = "#ffffff";
        ctx.fillText(colors[i].name + " " + colors[i].stock, cx + 16, cy + 30);
        cx += widths[i] + 48;
      }
    }
    // 底部
    ...
    ctx.fillText("— 羊毛衫管家 · 微信下单 · 现货速发 —", W / 2, H - 44);
