...
const CHANGELOG = {
  ...
  "3.29": [
    "界面质感再打磨：勾选框品牌红、背景暖光晕、今日统计加¥符号、快捷图标按压弹跳、卡片桌面悬浮感"
  ],
  "3.28": [
    "新增「每日明细」：统计页一键进入，按天查看每天的营业额、利润、单数、件数，点某天看当天明细"
  ],
  "3.27": [
    "分享海报价格重叠彻底修复：价格块改为左对齐精确排布（之前居中对齐导致 ¥ 和数字叠在一起）"
  ],
  "3.26": [
    "分享海报修复：¥ 和价格数字同一基线不再叠在一起、价格与款式名严格居中对齐、名称下方加细分隔线更美观"
  ],
  "3.25": [
    "升级秒级同步：手机 A 记账后 1.2 秒上传，手机 B 每 5 秒自动检测云端变化并拉取，另一台手机的数据几秒内就显示",
    "同步更省流量：每 5 秒只查几十字节的时间戳，有变化才下载数据"
  ],
  "3.24": [
...
/** 本会话自己上传过的时间戳（ms），用于秒级同步识别"自己上传"避免拉回环 */
const _myUploads = new Set();
function rememberUpload(ts) {
  try {
    const ms = Date.parse(ts);
    if (!isNaN(ms)) {
      _myUploads.add(ms);
      if (_myUploads.size > 10) {
        const first = _myUploads.values().next().value;
        _myUploads.delete(first);
      }
    }
  } catch {}
}

/** 数据变更后自动上传到云端（防抖 1.2 秒），实现多设备秒级同步 */
...
function flushUpload() {
  if (!currentAccount || _flushSyncing) return;
  _flushSyncing = true;
  Sync.isConfigured().then((ok) => {
    if (ok) return Sync.upload().then((sent) => rememberUpload(sent));
...
/* 秒级自动同步：
 * - 每 5 秒本地重算今日统计
 * - 每 5 秒查一次云端 updated_at（几十字节，很省流量），变了才拉取全量并刷新当前视图
 */
let _todayRefreshTimer = null;
let _lastSeenSyncMs = 0; // 最近看到的云端更新时间（ms）
...
function startTodayAutoRefresh() {
  ...
  _todayRefreshTimer = setInterval(async () => {
    renderToday();
    if (!currentAccount || document.visibilityState !== "visible") return;
    try {
      if (!await Sync.isConfigured()) return;
      // 1) 轻量查询云端更新时间戳
      const rows = await Sync.request("/rest/v1/knit_sync?username=eq." + encodeURIComponent(currentAccount.username) + "&select=updated_at");
      const serverMs = (rows && rows[0] && rows[0].updated_at) ? Date.parse(rows[0].updated_at) : 0;
      if (!serverMs || serverMs === _lastSeenSyncMs) return;
      // 2) 自己刚上传的不拉（避免回环），其余情况拉取合并
      if (_myUploads.has(serverMs)) { _lastSeenSyncMs = serverMs; return; }
      _lastSeenSyncMs = serverMs;
      window.__suppressUpload = true;
...
function renderTodayReminders() {
  ...
  // 6. 今日未收款项（今日订单未收 + 今日赊账销售）
  let todayUnpaid = 0, todayUnpaidCount = 0;
  for (const o of ordersCache) {
    if (o.status !== "cancelled" && isToday(o.createdAt)) {
      const d = orderDebt(o);
      if (d > 0) { todayUnpaid += d; todayUnpaidCount++; }
    }
  }
  for (const s of salesCache) {
    if (s.onCredit && isToday(s.time)) todayUnpaid += s.qty * s.price;
  }
  if (todayUnpaid > 0) reminders.push({ icon: "🧾", text: `今日还有 ${money(todayUnpaid)} 未收（${todayUnpaidCount} 单）`, go: "today-unpaid" });
  // 7. 没发的货（待发货订单）
  const unshipped = ordersCache.filter((o) => o.status === "pending" && !o.shippedAt);
  if (unshipped.length) {
...
function renderDaily() {
  // 按天聚合：营业额 = 销售 + 订单（与今日统计口径一致）
  const map = {};
  const getDay = (k) => (map[k] || (map[k] = { sales: 0, profit: 0, orders: 0, qty: 0 }));
  for (const s of salesCache) {
    const d = getDay(dayKey(s.time));
    d.sales += (s.qty || 0) * (s.price || 0);
    d.profit += (s.profit || 0);
    d.qty += (s.qty || 0);
  }
  for (const o of ordersCache) {
    if (o.status === "cancelled") continue;
    const d = getDay(dayKey(o.createdAt));
    d.sales += orderTotal(o);
    d.profit += orderProfit(o);
    d.orders++;
    d.qty += orderTotalQty(o);
  }
  const days = Object.keys(map).sort().reverse();
  $("#daily-total-sales").textContent = money(days.reduce((s, k) => s + map[k].sales, 0));
  $("#daily-total-profit").textContent = money(days.reduce((s, k) => s + map[k].profit, 0));
  // 选中日的明细
  const det = $("#daily-detail");
  if (dailySelected && map[dailySelected]) {
    const dayOrders = ordersCache.filter((o) => o.status !== "cancelled" && dayKey(o.createdAt) === dailySelected);
    const daySales = salesCache.filter((s) => dayKey(s.time) === dailySelected);
    det.innerHTML = `<div class="card">
      <h3>📅 ${dailySelected} 明细</h3>
      ${dayOrders.map((o) => `<div class="detail-color-row"><span>${escapeHtml(o.customer)} · ${(o.lines || []).map((l) => l.qty + "×" + escapeHtml(l.itemName)).join("、")}${o.status === "done" ? "" : " · 未完成"}</span><span class="v">${money(orderTotal(o))}</span></div>`).join("")}
      ${daySales.map((s) => `<div class="detail-color-row"><span>${escapeHtml(s.itemName)}${s.color ? "(" + escapeHtml(s.color) + ")" : ""} ×${s.qty}${s.customer ? " · " + escapeHtml(s.customer) : ""}${s.onCredit ? " · 赊账" : ""}</span><span class="v">${money((s.qty || 0) * (s.price || 0))}</span></div>`).join("")}
      ${(!dayOrders.length && !daySales.length) ? '<div class="hint">当天没有记录</div>' : ""}
      <div class="hint" style="text-align:center">当天合计 ${money(map[dailySelected].sales)} · 利润 ${money(map[dailySelected].profit)}</div>
    </div>`;
  } else {
    det.innerHTML = "";
  }
  ...
  if (!days.length) {
    list.innerHTML = '<div class="empty"><span class="empty-icon">📅</span>还没有任何记录</div>';
    return;
  }
  list.innerHTML = days.map((k) => {
    const d = map[k];
    const sel = k === dailySelected ? ' style="border-color:var(--primary)"' : "";
    return `<div class="list-card" data-day="${k}"${sel}>
      <div class="list-main">
        <div class="list-title">${k.slice(5)}</div>
        <div class="list-sub">${d.orders} 单 · ${d.qty} 件</div>
      </div>
      <div class="list-side">
        <div class="num">${fmt(d.sales)}</div>
        <div class="lbl">营业额 · 利润 ${fmt(d.profit)}</div>
      </div>
    </div>`;
  }).join("");
  $$("#daily-list .list-card").forEach((c) => {
    c.onclick = () => { dailySelected = c.dataset.day; renderDaily(); };
  });
  staggerIn(list);
}
...
async function shareItemCard(it) {
  try {
    ...
    ctx.textAlign = "center";
    // 款式名（自动缩放防溢出）
    const nameSize = fitFont(ctx, it.name, W - 90, 54, 26);
    ctx.fillStyle = "#1c1917";
    ctx.fillText(it.name, W / 2, IMG_H + 98);
    // 名称下方细分隔线
    ctx.strokeStyle = "#f0eeec";
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(W / 2 - 130, IMG_H + 142);
    ctx.lineTo(W / 2 + 130, IMG_H + 142);
    ctx.stroke();
    // 价格：¥ 与数字按左对齐精确排布（textAlign 必须是 left，否则会以各自坐标为中心导致重叠）
    const priceStr = fmt(it.price);
    const numSize = fitFont(ctx, priceStr, W - 220, 92, 40);
    const yuanSize = Math.round(numSize * 0.5);
    const priceY = IMG_H + 230;
    ctx.textAlign = "left";
    ...
    const gap = 12;
    const startX = (W - (yuanW + gap + numW)) / 2;
    ctx.fillStyle = "#b91c2e";
    ctx.font = "bold " + yuanSize + "px sans-serif";
    ctx.fillText("¥", startX, priceY);
    ctx.fillStyle = "#b91c2e";
    ctx.font = "bold " + numSize + "px sans-serif";
    ctx.fillText(priceStr, startX + yuanW + gap, priceY);
    ctx.textAlign = "center";
    ...
    if (colors.length) {
      ...
      const cSize = fitFont(ctx, colorText, W - 90, 30, 20);
      ctx.fillStyle = "#78716c";
      ctx.fillText(colorText, W / 2, IMG_H + 320);
    }
    // —— 底部品牌条（工厂名称，自动缩放防溢出）——
...
function updatePurchaseColor() {
  ...
  selColor.innerHTML = (it && it.colors ? it.colors : []).map((c) =>
    `<option value="${escapeHtml(c.name)}">${escapeHtml(c.name)}（库存 ${c.stock}）</option>`).join("") +
    `<option value="__new__">＋ 新增颜色…</option>`;
}
...
async function confirmPurchase() {
  const it = itemsCache.find((x) => x.id === $("#pu-item").value);
  if (!it) return toast("请选择款式");
  // 颜色：可选现有颜色，或选"新增颜色"后输入新颜色名
  const newInp = $("#pu-color-new");
  let colorName = "";
  if (!newInp.classList.contains("hidden")) {
    colorName = newInp.value.trim();
    if (!colorName) return toast("请填写新颜色名称");
  } else {
    colorName = $("#pu-color").value;
  }
  const qty = Number($("#pu-qty").value);
  const unitPriceRaw = $("#pu-price").value.trim();
  const supplier = $("#pu-supplier").value.trim();
  if (!colorName) return toast("请选择颜色或新增颜色");
  if (!(qty >= 1)) return toast("请填写正确的送货数量");
  const unitPrice = unitPriceRaw === "" ? null : Number(unitPriceRaw);
  let color = findColor(it, colorName);
  if (!color) {
    // 新颜色：自动加入款式
    color = { name: colorName, stock: 0, sold: 0 };
    it.colors.push(color);
  }
  color.stock += qty;
  await Store.saveItem(it);
  await Store.savePurchase({ itemId: it.id, itemName: it.name, color: colorName, qty, unitPrice, supplier });
  toast(`${supplier ? "加工商「" + supplier + "」" : ""}已入库 ${qty} 件${colorName ? "（" + colorName + "）" : ""}`);
  // 重置控件
...
async function enterApp() {
  ...
  try {
    if (await Sync.isConfigured()) {
      ...
      await reloadAll();
      // 记录云端当前更新时间，秒级同步从这一刻开始
      try {
        const rows = await Sync.request("/rest/v1/knit_sync?username=eq." + encodeURIComponent(currentAccount.username) + "&select=updated_at");
        const serverMs = (rows && rows[0] && rows[0].updated_at) ? Date.parse(rows[0].updated_at) : Date.now();
        _lastSeenSyncMs = isNaN(serverMs) ? Date.now() : serverMs;
      } catch {}
    }
  } catch (e) {
    // 同步失败不阻塞
...
function bindEvents() {
  ...
  // 每日营业额/利润明细入口
  $("#stats-daily-btn").onclick = () => { dailySelected = null; renderDaily(); showView("daily"); };
  ...
  // 选"＋ 新增颜色"时切到输入框
  $("#pu-color").onchange = () => {
    if ($("#pu-color").value === "__new__") {
      $("#pu-color").classList.add("hidden");
      const inp = $("#pu-color-new");
